...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  if (lengthVector1 === 0 || lengthVector2 === 0) return undefined;

  const dotProduct = x1 * x2 + y1 * y2;
  const angleBetweenVectors = Math.acos(
    dotProduct / (lengthVector1 * lengthVector2)
  );
...
function roundToPowerOfTen(num, pow) {
  const dividerRoundTo = 10 ** pow;
  const numberDivided = num / dividerRoundTo;
  const numberDividedRounded = Math.round(numberDivided);
  const numberOriginalRounded = numberDividedRounded * dividerRoundTo;

  return numberOriginalRounded;
}
...
function toNumber(value, def) {
  const valueToNumberTry = Number(value);
  const valueToReturn = Number.isNaN(valueToNumberTry) ? def : valueToNumberTry;
...
function getFibonacciNumber(index) {
  const getNextFibonacciNumber = (numberPrevious, numberPrePrevious) => {
    return numberPrevious + numberPrePrevious;
  };
  const fibonacciSequence = [0, 1];
  let i = 2;
  while (index > 1 && i <= index) {
    const previousNumber = fibonacciSequence[i - 1];
    const prePreviousNumber = fibonacciSequence[i - 2];
    const nextFibonacciNumber = getNextFibonacciNumber(
      previousNumber,
      prePreviousNumber
    );
  ...
  const fibonacciNumberForIndex = fibonacciSequence[index];

  return fibonacciNumberForIndex;
}
...
function getSumToN(n) {
  let sumAllPreviousNumbers = 0;
  let i = 0;
  while (i < n) {
    i += 1;
    sumAllPreviousNumbers += i;
  }
  return sumAllPreviousNumbers;
}
...
function getSumOfDigits(num) {
  ...
  let sumOfDigits = 0;
  for (let i = 0; i < stringFromNumber.length; i += 1) {
    sumOfDigits += +stringFromNumber[i];
  }
  return sumOfDigits;
}
...
function isPowerOfTwo(num) {
  let isNumberPowerOfTwo = true;
  const isNumberDividedByTwo = num % 2 === 0;
  if (!isNumberDividedByTwo) {
    isNumberPowerOfTwo = false;
  }

  let remainderValue = num;
  while (isNumberPowerOfTwo && remainderValue !== 2) {
    remainderValue /= 2;
    const isRemainderDividedByTwo = remainderValue % 2 === 0;
    if (!isRemainderDividedByTwo) {
      isNumberPowerOfTwo = false;
    }
  }

  return isNumberPowerOfTwo;
}
